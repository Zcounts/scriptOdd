...
function deriveScenes(json, existingScenes) {
  ...
  for (const block of blocks) {
    if (block.type === 'sceneHeading') {
      const id = sceneIdFromAttrs(block.attrs)
      if (!id) continue
...
test('deriveScenes: extracts scene from sceneHeading block', () => {
  const json = {
    type: 'doc',
    content: [
      { type: 'sceneHeading', attrs: { id: 'h1', sceneId: 'sc1', tags: [], noteIds: [] }, content: [{ type: 'text', text: 'INT. OFFICE - DAY' }] },
    ],
  }
...
test('deriveScenes: preserves synopsis and color from existing scenes', () => {
  const json = {
    type: 'doc',
    content: [
      { type: 'sceneHeading', attrs: { id: 'h1', sceneId: 'sc1', tags: [], noteIds: [] }, content: [{ type: 'text', text: 'INT. A - DAY' }] },
    ],
  }
  const existing = [{ id: 'sc1', heading: 'INT. A - DAY', synopsis: 'The setup', color: 'blue', order: 0, noteIds: ['n1'] }]
...
test('deriveScenes: ignores blocks that are not sceneHeadings', () => {
  const json = {
    ...
    content: [
      ...
      { type: 'sceneHeading', attrs: { id: 'h2', sceneId: 'sc2', tags: [], noteIds: [] }, content: [{ type: 'text', text: 'EXT. PARK' }] },
    ],
  }
  const scenes = deriveScenes(json, [])
  assert.equal(scenes.length, 1)
...
test('deriveScenes: skips sceneHeadings with no resolvable ID', () => {
  const json = {
    type: 'doc',
    content: [
      { type: 'sceneHeading', attrs: { id: null, sceneId: null, tags: [], noteIds: [] }, content: [{ type: 'text', text: 'INT. ROOM - DAY' }] },
    ],
  }
  const scenes = deriveScenes(json, [])
...
test('deriveScenes: falls back to attrs.id when sceneId is null', () => {
  const json = {
    type: 'doc',
    content: [
      { type: 'sceneHeading', attrs: { id: 'fallback-id', sceneId: null, tags: [], noteIds: [] }, content: [{ type: 'text', text: 'INT. X' }] },
    ],
  }
  const scenes = deriveScenes(json, [])
...
test('deriveScenes: assigns sequential order values', () => {
  const json = {
    type: 'doc',
    content: [
      { type: 'sceneHeading', attrs: { id: 'h1', sceneId: 'sc1' }, content: [{ type: 'text', text: 'Scene 1' }] },
      { type: 'sceneHeading', attrs: { id: 'h2', sceneId: 'sc2' }, content: [{ type: 'text', text: 'Scene 2' }] },
      { type: 'sceneHeading', attrs: { id: 'h3', sceneId: 'sc3' }, content: [{ type: 'text', text: 'Scene 3' }] },
    ],
  }
  const scenes = deriveScenes(json, [])
  assert.equal(scenes[0].order, 0)
